...
class Signin extends Backend
{
    ...
    {
        $row = $this->model->get(['id', $league_signin_id]);
        $league_ids = League::where('id', 'in', $row->getdata('league_ids'))->column('name');
        $user_id = \app\common\model\User::where($row->getAttr('user_id'))->value('nickname');
        $row->visible(['user_id', 'name', 'daterange', 'address', 'remark', 'league_ids']);
