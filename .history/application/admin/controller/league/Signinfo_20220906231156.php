        ...
        if ($this->request->isAjax()) {
            ...
            foreach($all_ids as $v) {
                if(!in_array($v,$sign_ids)){
                    $user = $this->model->get($v)->append([],true)->visible(['nickname','mobile'])->toArray();
                    array_push($unsign_user,$user);
                }
            }
            //$this->view->assign("row",$unsign_user);
            $result = array("total" => count($unsign_user), "rows" => $unsign_user);
            //dump($result);

            return json($result);
        }
        return $this->view->fetch('signed');
    }
    ...
    public function signin($league_signin_id = '')
    {
        if ($this->request->isPost()) {
            if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
                $channel = 'WeChat';
                $address = '';
                $this->add($league_signin_id, $channel, $address, 3);
            } else {
                $this->error('请在微信中签到！');
            }
        }
